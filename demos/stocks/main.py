...
class Stocks:
    """This program displays a stock market ticker and cycles through multiple stocks
    and creates a graph of their last day of trading. It makes a request using the
    AlphaAdvantage API when the data is out of date. It reads and writes this data
    to data.json."""
...
def setData(symbol, numbers):
    ...
    close = numbers[1]
    date = numbers[2]
    with open("demos/stocks/data.json", "r+") as f:
        data = json.load(f)
        temp = data["symbols"]
        date = getDate()
        try:
